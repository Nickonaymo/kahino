...
# Fix: Refresh selected items display after removal
...
# Fix: Add new item or increase quantity dynamically
...
# Fix: Display selected items after clicking 'Place Order'
def display_chosen_items():
    global selected_items_panel, sub_total, delivery_charge, total, Orders, place_order
    ...
    place_order = Button(dash_board,
           ...
           font=('Arial', 15, 'bold'),
           border=0,
           command=lambda: print("Order Placed"))  # Just a placeholder action for now
    place_order.place(x=840, y=460)
...
# Function to hide the chosen items section
...
def toggle_password_visibility(password, show_password_var):
    if show_password_var.get():
        password.config(show='')  # Show password (text input)
    else:
        password.config(show='*')  # Hide password (show '*' characters)s)

# Login Display
def login_display():
    hide_display_chosen_items()
    ...
    # Define BooleanVar to track the checkbox state
    ...
    show_password_checkbox = Checkbutton(
        ...
        command=lambda: toggle_password_visibility(password, show_password_var)  # Pass password widget here
    )
    show_password_checkbox.place(x=10, y=214)

    # Forgot Password Button
    forgot_password = Button(login_frame, width=20, text='Forgot Password?', border=0, bg='light gray', font=('Arial', 8),cursor='hand2', fg='#DB7921')
    forgot_password.place(x=120, y=216)

    # Sign in Button
    Button(login_frame, width=33, pady=10, text='Sign in', bg='#DB7921', fg='white', border=0).place(x=10,y=250)


#Create the buttons using the function
create_button('Login.png', 'Login', 100, 120, command=login_display)
